...
public class GT_TileEntity_ComputerCube extends GT_MetaTileEntity_BasicMachine implements IReactor {
        ...
        for (int i = 54; i < 58; i++) {
            if (mInventory[i] != null) {
                if (!this.getWorld().isRemote)
                    this.getWorld().spawnEntityInWorld(new EntityItem(this.getWorld(), this.getBaseMetaTileEntity().getXCoord() + 0.5, this.getBaseMetaTileEntity().getYCoord() + 0.5, this.getBaseMetaTileEntity().getZCoord() + 0.5, mInventory[i]));
                mInventory[i] = null;
            }
        }
    }
    ...
    private void switchMode() {
        ...
        this.getWorld().addBlockEvent(this.getBaseMetaTileEntity().getXCoord(), this.getBaseMetaTileEntity().getYCoord(), this.getBaseMetaTileEntity().getZCoord(), GregTech_API.sBlockList[1].blockID, 10, mMode);
        this.getWorld().addBlockEvent(this.getBaseMetaTileEntity().getXCoord(), this.getBaseMetaTileEntity().getYCoord(), this.getBaseMetaTileEntity().getZCoord(), GregTech_API.sBlockList[1].blockID, 11, mMaxHeat);
    }
    ...
    public void showDescription(int aIndex) {
        ...
        if (aIndex >= GT_ComputercubeDescription.sDescriptions.size() || aIndex < 0)
            aIndex = 0;
        if (GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[0] == null)
            mInventory[59] = null;
        else
            mInventory[59] = GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[0].copy();

        if (GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[1] == null)
            mInventory[60] = null;
        else
            mInventory[60] = GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[1].copy();
        if (GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[2] == null)
            mInventory[61] = null;
        else
            mInventory[61] = GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[2].copy();
        if (GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[3] == null)
            mInventory[62] = null;
        else
            mInventory[62] = GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[3].copy();
        if (GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[4] == null)
            mInventory[63] = null;
        else
            mInventory[63] = GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[4].copy();
        if (GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[5] == null)
            mInventory[64] = null;
        else {
            mInventory[64] = GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[5].copy();
            mExplosionStrength = 100.0F;
        }
        if (GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[6] == null)
            mInventory[65] = null;
        else {
            mInventory[65] = GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[6].copy();
            mExplosionStrength = 100.0F;
        }
        if (GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[7] == null)
            mInventory[66] = null;
        else {
            mInventory[66] = GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[7].copy();
            mExplosionStrength = 100.0F;
        }
        if (GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[8] == null)
            mInventory[67] = null;
        else {
            mInventory[67] = GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[8].copy();
            mExplosionStrength = 100.0F;
        }
        if (GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[9] == null)
            mInventory[68] = null;
        else {
            mInventory[68] = GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[9].copy();
            mExplosionStrength = 100.0F;
        }
        if (GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[10] == null)
            mInventory[69] = null;
        else {
            mInventory[69] = GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[10].copy();
            mExplosionStrength = 100.0F;
        }
        if (GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[11] == null)
            mInventory[70] = null;
        else {
            mInventory[70] = GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[11].copy();
            mExplosionStrength = 100.0F;
        }
        if (GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[12] == null)
            mInventory[71] = null;
        else {
            mInventory[71] = GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[12].copy();
            mExplosionStrength = 100.0F;
        }
        if (GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[13] == null)
            mInventory[72] = null;
        else {
            mInventory[72] = GT_ComputercubeDescription.sDescriptions.get(aIndex).mStacks[13].copy();
            mExplosionStrength = 100.0F;
        }
        mMaxHeat = aIndex;
        this.getWorld().addBlockEvent(this.getBaseMetaTileEntity().getXCoord(), this.getBaseMetaTileEntity().getYCoord(), this.getBaseMetaTileEntity().getZCoord(), GregTech_API.sBlockList[1].blockID, 11, mMaxHeat);
    ...
    public void showCentrifugeRecipe(int aIndex) {
        ...
        if (tRecipe != null) {
            if (tRecipe.mInput1 == null) mInventory[59] = null;
            else mInventory[59] = tRecipe.mInput1.copy();
            if (tRecipe.mInput2 == null) mInventory[60] = null;
            else mInventory[60] = tRecipe.mInput2.copy();
            if (tRecipe.mOutput1 == null) mInventory[61] = null;
            else mInventory[61] = tRecipe.mOutput1.copy();
            if (tRecipe.mOutput2 == null) mInventory[62] = null;
            else mInventory[62] = tRecipe.mOutput2.copy();
            if (tRecipe.mOutput3 == null) mInventory[63] = null;
            else mInventory[63] = tRecipe.mOutput3.copy();
            if (tRecipe.mOutput4 == null) mInventory[64] = null;
            else mInventory[64] = tRecipe.mOutput4.copy();
    ...
    public void onPostTickUpdate() {
        if (!this.getWorld().isRemote) {
            ...
            if (mMode == 1 && GT_Mod.mReactorplanner && mStarted && decreaseStoredEnergy(32, false))
                for (int i = 0; i < 25 && mStarted; i++) {
                    mEUOut = 0;
                    mMaxHeat = 10000;
                    mHEM = 1.0F;
    ...
    @Override
    public float addOutput(float v) {
        return 0;
    }
    ...
    @Override
    public void setRedstoneSignal(boolean b) {
